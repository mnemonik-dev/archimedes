...
def monte_carlo_dsr_pvalue(
    ...
    """Block-bootstrap p-value for the Deflated Sharpe Ratio.

    Assesses whether the observed annualized Sharpe ratio is statistically
    distinguishable from a bootstrap null generated by resampling the original
    return series via circular block bootstrap (Politis & Romano 1992). The
    block structure preserves the short-range serial dependence typically
    present in daily financial returns, unlike i.i.d. resampling which assumes
    independence.

    Algorithm:
      1. Draw *n_trials* circular-block-bootstrap replicates of the return series.
      2. Compute the annualized Sharpe ratio for each replicate.
      3. Empirical p-value = fraction of bootstrap Sharpes ≥ the observed Sharpe.
         A low p-value (e.g. < 0.05) means the observed Sharpe is unlikely under
         the null of "no skill beyond what the data's own autocorrelation structure
         can produce by chance".
      4. The *dsr_threshold* parameter is recorded in the return dict for the
         caller but is not used internally (the observed Sharpe is derived from
         *returns* directly).
    ...
    the Limits of Bootstrap* (pp. 263-270). Wiley, New York.
    Application to Sharpe-ratio significance: see Bailey et al. (2014) for
    the broader multiple-testing context.

    Args:
        returns: Daily (per-bar) return series, un-annualized.
        dsr_threshold: The DSR significance threshold being assessed (e.g. 0.95).
            Stored in the output dict for caller reference; not used in the
    ...
    Returns:
        Dict with keys:
          ``pvalue`` — empirical fraction of bootstrap Sharpes ≥ observed Sharpe.
          ``bootstrap_sharpe_mean`` — mean of the bootstrap Sharpe distribution.
          ``bootstrap_sharpe_std`` — std of the bootstrap Sharpe distribution.
    ...
    bs_samples = _circular_block_bootstrap(arr, block_size=block_size, n_trials=n_trials, rng=rng)
    # shape: (n_trials, T)
    ...
    # Empirical p-value: P(bootstrap Sharpe >= observed Sharpe) under H0
