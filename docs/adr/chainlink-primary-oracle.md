...
1. **Where a well-validated feed exists, it is PRIMARY.** "Well-validated" means Chainlink or an equivalent generally-accepted, audited, decentralized oracle exposing the canonical `AggregatorV3Interface` (Arc-native / Chainlink-compatible aggregators qualify). When `priceFeed != address(0)`, `getPrice()` reads the feed and the admin-fed `price` storage is **not consulted** for the live read.

2. **The feed read fails closed.** `_readChainlink()` validates, in order: `answer > 0` (`NegativePrice`), `updatedAt != 0` (`IncompleteRound`), `answeredInRound >= roundId` (`StaleFeedRound`), and `block.timestamp - updatedAt <= MAX_STALENESS` (`StalePrice`) — the *same* staleness bound the admin path uses. Any failure reverts. Consumers treat a revert as "do not trade," so a bad or stale feed **blocks rather than misprices**. Feed answers are scaled to 6 decimals (8-decimal USD feeds → `/100`); `setPriceFeed` validates `decimals() <= 36` at config time so the scaling math can't overflow on read.
...
- **Fail-closed by construction.** A degraded feed reverts; the system declines to trade rather than trading on a bad price. That is the correct default for funds-adjacent reads.
...
**Dan's concern, stated plainly: when Chainlink goes offline, precedence is preserved but the feed read fails closed — so for that asset we either halt (feed configured, now stale → revert) or, if an operator has deliberately cleared the feed to restore liveness, we lean on the *less-hardened* admin fallback.** That is a real reduction in trust-minimization exactly at the moment of stress, and it is the price of choosing liveness over strict-only.
