...
ReasoningTraceRegistry gated + commit-reveal *(Correction, 2026-06-14: commit-reveal
exists in `ReasoningTraceRegistry.sol` source but is NOT invoked from the backend —
`agent_runner.py`/`trace_publisher.py` still call only the legacy `publishTrace`; this
was a partial/in-progress item, not fully "verified genuinely fixed." See `AUDIT_2026-06-14.md`
finding B2.)* · AMMPool k-invariant (correct) · all
